...
class DataLayer(object):
    ...
    def augument(self, images):
        ...
        random_brightness = tf.random.uniform([], 0.8, 1.2)
    ...
    def scale(self, id, images, poses, depth_gt, filled, intrinsics):
        ...
        if len(cfg.INPUT.SCALES) > 1:
            ...
            images = tf.image.resize(images, [ht1, wd1], method=tf.image.ResizeMethod.BILINEAR)[:, dy:dy+ht, dx:dx+wd]
            depth_gt = tf.image.resize(depth_gt[tf.newaxis], [ht1, wd1], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)[:, dy:dy+ht, dx:dx+wd]
            filled = tf.image.resize(filled[tf.newaxis], [ht1, wd1], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)[:, dy:dy+ht, dx:dx+wd]
    ...
    def flip(self, id, images, poses, depth_gt, filled, intrinsics):
        ...
        images = tf.cond(pred=do_flip, true_fn=lambda: images, false_fn=lambda: images[:, :, ::-1])
        depth_gt = tf.cond(pred=do_flip, true_fn=lambda: depth_gt, false_fn=lambda: depth_gt[:, ::-1])
        filled = tf.cond(pred=do_flip, true_fn=lambda: filled, false_fn=lambda: filled[:, ::-1])

        wd = tf.cast(tf.shape(input=images)[2], dtype=tf.float32)
        fx, fy, cx, cy = tf.unstack(intrinsics, num=4, axis=-1)
        intrinsics_flipped = tf.stack([-fx, fy, wd - cx, cy], axis=-1)
        intrinsics = tf.cond(pred=do_flip, true_fn=lambda: intrinsics, false_fn=lambda: intrinsics_flipped)

        flip_offset = tf.constant(FLIP_OFFSET, dtype=tf.int32)
        id = tf.cond(pred=do_flip, true_fn=lambda: id, false_fn=lambda: id + flip_offset)
    ...
    def read_example(self, tfrecord_serialized):
        ...
        tfrecord_features = tf.io.parse_single_example(serialized=tfrecord_serialized,
            features={
                'id': tf.io.FixedLenFeature([], tf.string),
                'dim': tf.io.FixedLenFeature([], tf.string),
                'images': tf.io.FixedLenFeature([], tf.string),
                'poses': tf.io.FixedLenFeature([], tf.string),
                'depth': tf.io.FixedLenFeature([], tf.string),
                'filled': tf.io.FixedLenFeature([], tf.string),
                'intrinsics': tf.io.FixedLenFeature([], tf.string),
            }, name='features')

        id = tf.io.decode_raw(tfrecord_features['id'], tf.int32)
        dim = tf.io.decode_raw(tfrecord_features['dim'], tf.int32)

        images = tf.io.decode_raw(tfrecord_features['images'], tf.uint8)
        poses = tf.io.decode_raw(tfrecord_features['poses'], tf.float32)
        depth = tf.io.decode_raw(tfrecord_features['depth'], tf.float32)
        filled = tf.io.decode_raw(tfrecord_features['filled'], tf.float32)
        intrinsics = tf.io.decode_raw(tfrecord_features['intrinsics'], tf.float32)
        ...
        if cfg.INPUT.SAMPLES > 0:
            ix = tf.compat.v1.py_func(random_frame_idx, [cfg.INPUT.FRAMES, cfg.INPUT.SAMPLES], tf.int32)
            ix = tf.reshape(ix, [cfg.INPUT.SAMPLES+1])

            images = tf.gather(images, ix, axis=0)
            poses = tf.gather(poses, ix, axis=0)

        do_augument = tf.random.uniform([], 0, 1)
        images = tf.cond(pred=do_augument<0.5, true_fn=lambda: self.augument(images), false_fn=lambda: images)

        id, images, poses, depth, filled, intrinsics = \
            self.scale(id, images, poses, depth, filled, intrinsics)

        prediction = tf.compat.v1.py_func(_read_prediction_py, [id, filled], tf.float32)
    ...
    def next(self):
        ...
        tfreader = tf.compat.v1.TFRecordReader()
        ...
        id, images, poses, depth_gt, filled, depth_pred, intrinsics \
            = tf.compat.v1.train.batch(example, batch_size=self.batch_size, num_threads=2, capacity=64)
    ...
    def write(self, id, prediction):
        return tf.compat.v1.py_func(_write_prediction_py, [id, prediction], tf.int32)
...
def scale(id, images, poses, depth_gt, filled, pred, intrinsics):
    ...
    if len(cfg.INPUT.SCALES) > 1:
        ...
        images = tf.image.resize(images, [ht1, wd1], method=tf.image.ResizeMethod.BILINEAR)[:, dy:dy+ht, dx:dx+wd]
        depth_gt = tf.image.resize(depth_gt, [ht1, wd1], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)[:, dy:dy+ht, dx:dx+wd]
        filled = tf.image.resize(filled, [ht1, wd1], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)[:, dy:dy+ht, dx:dx+wd]
...
class DBDataLayer:
    def __init__(self, db, batch_size=1, augument=False):
        ...
        self.training_iterator = tf.compat.v1.data.make_initializable_iterator(training_set)
