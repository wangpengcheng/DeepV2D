...
def dilated_conv2d(x, dim, stride=1, my_rate=6, bn=True):
    ...
    """
    if bn:
        return slim.conv2d(bnrelu(x), dim, [3, 3],rate=my_rate, stride=stride, scope='2d_rate{}'.format(my_rate))
    else:
        return slim.conv2d(tf.nn.relu(x), dim, [3, 3],rate=my_rate, stride=stride, scope='2d_rate{}'.format(my_rate))
