...
def compute_weights_reg_loss(ws, k=2048):
    ...
    iters, batch, frames, ht, wd, _ = tf.unstack(tf.shape(input=ws), num=6)
    ws = tf.reshape(ws, [iters*batch*frames, ht, wd, 2])
    ws = tf.transpose(a=ws, perm=[0, 3, 1, 2])
...
class MotionNetwork:
    ...
    def pose_regressor_init(self, images):
        cfg = self.cfg
        batch, frames = [tf.shape(input=images)[i] for i in range(2)]
    ...
    def extract_features(self, images, reuse=False):
        batch, frames, _, _ = [tf.shape(input=images)[i] for i in range(4)]
    ...
    def flownet(self, fmap1, fmap2, reuse=False):
        ...
        batch, num, height, width = [tf.shape(input=inputs)[i] for i in range(4)]
        ...
        with tf.compat.v1.variable_scope('flow_net') as sc:
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                tf.compat.v1.add_to_collection("checkpoints", cnv1)

                cnv1b = slim.conv2d(cnv1,  32,  [7, 7], stride=1, scope='cnv1b')
                cnv2  = slim.conv2d(cnv1b, 64,  [5, 5], stride=2, scope='cnv2')
                tf.compat.v1.add_to_collection("checkpoints", cnv1b)

                cnv2b = slim.conv2d(cnv2,  64,  [5, 5], stride=1, scope='cnv2b')
                cnv3  = slim.conv2d(cnv2b, 128, [3, 3], stride=2, scope='cnv3')
                tf.compat.v1.add_to_collection("checkpoints", cnv2b)

                cnv3b = slim.conv2d(cnv3,  128, [3, 3], stride=1, scope='cnv3b')
                cnv4  = slim.conv2d(cnv3b, 256, [3, 3], stride=2, scope='cnv4')
                tf.compat.v1.add_to_collection("checkpoints", cnv3b)

                cnv4b = slim.conv2d(cnv4,  256, [3, 3], stride=1, scope='cnv4b')
                cnv5  = slim.conv2d(cnv4b, 512, [3, 3], stride=2, scope='cnv5')
                tf.compat.v1.add_to_collection("checkpoints", cnv4b)
        ...
        tf.compat.v1.add_to_collection("checkpoints", flow)
        tf.compat.v1.add_to_collection("checkpoints", weight)
    ...
    def forward(self, Ts, images, depths, intrinsics, inds=None, num_fixed=0, init=tf.constant(False)):
        ...
        with tf.compat.v1.variable_scope("motion", reuse=self.reuse) as sc:
            if Ts is None:
            ...
            for i in range(cfg.FLOWSE3.ITER_COUNT):
                ...
                with tf.compat.v1.name_scope("residual"):
                ...
                with tf.compat.v1.name_scope("PnP"):
    ...
    def compute_loss(self, Gs, depths, intrinsics, loss='l1', log_error=True):
        ...
        for i in range(len(self.transform_history)):
            ...
            if loss == 'l1':
                ...
                reproj_loss = tf.reduce_mean(input_tensor=reproj_diff)

                w = tf.nn.sigmoid(self.weights_history[i-1])
                resid = tf.clip_by_value(self.residual_history[i], -MAX_ERROR, MAX_ERROR)
                
                resid_loss = tf.reduce_mean(input_tensor=valid_mask * w * resid**2)
                total_loss += reproj_loss + self.cfg.TRAIN.RESIDUAL_WEIGHT * resid_loss

                if log_error:
                    with tf.device('/cpu:0'):
                        self.summaries.append(tf.compat.v1.summary.scalar("reproj_loss", reproj_loss))
                        self.summaries.append(tf.compat.v1.summary.scalar("resid_loss", resid_loss))
