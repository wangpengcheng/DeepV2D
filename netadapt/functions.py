...
def extract_feature_map_sizes(model, input_data_shape):
    '''
        ...
        Input:
            ...
            `is_args`:模型是否为多参数输入，输入为多个参数时为，主要的参数形状
        Output:
    ...
    model = model.cuda()
    model.eval()
    if isinstance(input_data_shape, list) and isinstance(input_data_shape[0], list): 
        is_args = True
    else:
        is_args = False
    ...
    if is_args:
        model_args = []
        for temp_shape in input_data_shape:
            temp_data = torch.randn([1, *temp_shape]).cuda()
            model_args.append(temp_data)
        # 进行模型推理
        _ = model(*model_args)
    else:
        # 在这里执行一次基本操作
            ...
            if layer_type_str in FC_LAYER_TYPES:
                ...
                if previous_out_channels is None:
                    ...
                    if previous_out_channels % layer.in_channels != 0:
                        ...
                        if previous_out_channels < layer.in_channels :
                            before_squared_pixel_shuffle_factor = int(1)
                        else:
    ...
    for i in range(runtimes):       
        if is_cuda:
            if isinstance(input_data_shape, list) and isinstance(input_data_shape[0], list): 
                ...
                input = []
                for temp_shape in input_data_shape:
                    temp_input = torch.cuda.FloatTensor(*temp_shape).normal_(0, 1)
                    temp_input = temp_input.cuda(cuda_num)
            ...
            if isinstance(input_data_shape, list) and isinstance(input_data_shape[0], list): 
                ...
                input = []
                for temp_shape in input_data_shape:
                    temp_input = torch.randn(temp_shape)
                    input.append(temp_input)
                with torch.no_grad():
                    start = time.time()
                    model(*input)
...
def simplify_network_def_based_on_constraint(network_def, 
                                            block, constraint, resource_type,
            ...
            if kept_filter_idx is None:
                ...
                if layer_param_name == WEIGHTSTRING: #WEIGHTSTRING == layer_param_name:                    
                    if layer.groups == 1:  # Pointwise layer or depthwise layer with only one filter.
                        setattr(layer, layer_param_name,
                                torch.nn.Parameter(getattr(layer, layer_param_name)[:, kept_filter_idx.type(torch.long), :, :]))
                ...
                if layer_param_name == BIASSTRING:
                    ...
                    if num_in_features > len(kept_filter_idx):
                        ...
                        setattr(layer, layer_param_name, 
                                torch.nn.Parameter(getattr(layer, layer_param_name)[:, kept_filter_idx_fc.type(torch.long)]))
                ...
                if any(substr == layer_param_name for substr in [WEIGHTSTRING, BIASSTRING]):
                    setattr(layer, layer_param_name,
                            torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx.type(torch.long)], requires_grad=True))
                    ...
                    setattr(layer, layer_param_name,
                            torch.nn.Parameter(getattr(layer, layer_param_name)[kept_filter_idx.type(torch.long)], requires_grad=False))
                    layer.num_features = len(kept_filter_idx)
                    print('    simplify_model> simplify {} layer {}: {} {}'.format(layer_type_str,
                          layer_name_str, layer_param_name, layer.num_features))
